...
describe('CreateProductoDto - validaciones CR-001', () => {
  ...
  describe('precio (CA1)', () => {
    it('acepta precio > 0', async () => {
      expect(await propiedadesConError(buildPayload({ precio: 0.01 }))).toEqual([]);
    });

    it.each([0, -1])('rechaza precio = %p', async (precio) => {
      const errores = await validar(buildPayload({ precio }));
      expect(errores).toHaveLength(1);
      expect(errores[0].property).toBe('precio');
      expect(errores[0].constraints).toHaveProperty(
        'isPositive',
  ...
  describe('denominacion / nombre (CA3)', () => {
    it.each(['', '   '])('rechaza denominacion vacía (%p)', async (denominacion) => {
      const errores = await validar(buildPayload({ denominacion }));
      expect(errores.map((e) => e.property)).toEqual(['denominacion']);
      expect(errores[0].constraints).toHaveProperty(
        'isNotEmpty',
    ...
    it('rechaza denominacion ausente', async () => {
      const { denominacion, ...sinDenominacion } = buildPayload();
      expect(await propiedadesConError(sinDenominacion)).toEqual(['denominacion']);
    });

    // Comportamiento real: el @Transform ejecuta value.trim() sin proteger null,
    // por lo que falla antes de llegar a class-validator.
    it('con denominacion null, el @Transform lanza TypeError antes de validar', () => {
      expect(() =>
